angular.module("app", []).controller("consoleCtrl", function($scope, $timeout, $injector, $location, $http) {
    switch ($scope.doFunc = function(action, arguments) {
        angular.isFunction($scope[action]) ? $scope[action](arguments) : console.log("" + action + " is not a function");
    }, $scope.menu = {
        text: "",
        actionitems: [ {
            name: "home",
        ...
        var f3 = function() {
            $scope.addLine("[visitor@HVeeman " + $scope.path + "] $> ", "(click any of the listed directories to navigate)", 20, callback || null);
        }, f2 = function() {
            var index = $scope.content.length;
            $scope.content[index] = $scope.menu, f3();
        }, f1 = function() {
            $scope.addLine("[visitor@HVeeman " + $scope.path + "] $> ", "ls ", 300, f2);
        };
        f1();
    }, $scope.github = function() {
        $scope.content = [];
        var f1 = function() {
            $http.get("http://api.github.com/users/hartgerv").then(function(data) {
                var arr = Object.keys(data.data).map(function(key) {
        ...
        function calculateAge(dateString) {
            var today = new Date(), birthDate = new Date(dateString), age = today.getFullYear() - birthDate.getFullYear(), m = today.getMonth() - birthDate.getMonth();
            return (0 > m || 0 === m && today.getDate() < birthDate.getDate()) && age--, age;
        }
        $scope.content = [];
        var f2 = function() {
            $scope.addLines("", [ "{", '"name" : "Hartger Veeman",', '"occupation": "Software Engineering Student",', '"age": ' + calculateAge("1992-6-18") + ", ", '"location": "Groningen, the Netherlands",', '"contact": "HartgerVeeman@gmail.com",', '"description": "Hi! I am Hartger, a software engineering student from Groningen, the Netherlands. To me the field of IT is an ever evolving landscape. The rapid progression of technologies inspires me. I would like to be part of this progression. During my studies I work on developing a broad set of skills. I aim to be a flexible professional that can adapt to a fast changing work-field. Besides software engineering, the main focus of my education, I also take part in interdisciplinary courses and workshops. I have chaired multiple committees in my student union, which helped me grow my organisational skills.Right now I am looking for opportunities to learn and further develop my skills in the field of software engineering and web development. During my studies I am available for a part time job." } ', '"interests": [', '"front-end webdevelopment",', '"back-end webdevelopment",', '"artificial intelligence"', "],", '"skills": [', '{"Javascript": [', '"Front-end and back-end webdevelopment in javascript",', '"AngularJS",', '"jQuery",', '"NodeJS"', "]},", '{"Php": [', '"Laravel"', "]},", '{"Java": [', '"OOP"', "]},", '{"C": [', '"Imperative programming"', "]},", '{"Misc.: ["', '"HTML",', '"CSS",', '"Linux"', "]}", "]", '"Hobbies": ["music production","philosophy"],', "} " ], 0, $scope.showMenu);
        }, f1 = function() {
            $scope.addLine("[visitor@HVeeman " + $scope.path + "] $> ", "cat profile.json", 150, f2), 
            $scope.path = "app/public";
        };
        f1();
    }, $scope.websites = function() {
        $scope.content = [];
        var f11 = function() {
            $scope.addLine("[visitor@HVeeman " + $scope.path + "] $> ", "(click any of the listed directories to navigate)", 20);
        }, f10 = function() {
            var index = $scope.content.length;
            $scope.content[index] = $scope.menu, f11();
        }, f9 = function() {
            $scope.addLine("[visitor@HVeeman " + $scope.path + "] $> ", "ls", 300, f10);
        }, f8 = function() {
            $scope.addLine("[visitor@HVeeman " + $scope.path + "] $> ", "cd ..", 300, f9), $scope.path = "~/app/public";
        }, f7 = function() {
            var index = $scope.content.length;
            $scope.content[index] = {
                text: "Everymusic, a soundcloud based online music player (WIP): ",
                actionitems: [ {
                    name: "http://www.everymusic.nl",
                    action: "redirectTo",
                    arguments: "http://www.everymusic.nl"
                } ]
            }, f8();
        }, f6 = function() {
            var index = $scope.content.length;
            $scope.content[index] = {
                text: "Stackoverflow: ",
                actionitems: [ {
                    name: "http://stackoverflow.com/users/4550627/hartger",
                    action: "redirectTo",
                    arguments: "http://stackoverflow.com/users/4550627/hartger"
                } ]
            }, f7();
        }, f5 = function() {
            var index = $scope.content.length;
            $scope.content[index] = {
                text: "Github: ",
                actionitems: [ {
                    name: "https://github.com/HartgerV/",
                    action: "redirectTo",
                    arguments: "https://github.com/HartgerV/"
                } ]
            }, f6();
        }, f4 = function() {
            var index = $scope.content.length;
            $scope.content[index] = {
                text: "LinkedIn: ",
                actionitems: [ {
                    name: "https://nl.linkedin.com/in/hartgerveeman",
                    action: "redirectTo",
                    arguments: "https://nl.linkedin.com/in/hartgerveeman"
                } ]
            }, f5();
        }, f3 = function() {
            $scope.addLine(" ", "My presence on the internet:", 80, f4);
        }, f2 = function() {
            $scope.addLine("[visitor@HVeeman " + $scope.path + "] $> ", "consume <(cat personalprojects schoolprojects socialmedia)", 80, f3);
        }, f1 = function() {
            $scope.addLine("[visitor@HVeeman " + $scope.path + "] $> ", "cd ~/app/public/websites", 100, f2), 
            $scope.path = "app/public/websites";
        };
        f1();
    }, $scope.home = function() {
        $scope.content = [];
        var f6 = function() {
            ...
            var index = $scope.content.length;
            $scope.content[index] = $scope.menu, f6();
        }, f4 = function() {
            $scope.addLine("[visitor@HVeeman " + $scope.path + "] $> ", "ls", 300, f5);
        }, f3 = function() {
            $scope.addLine('"Hi, I am Hartger! Welcome to my personal website. I am a software engineering student from Groningen, the Netherlands. This site contains information about me and the software engineering related things I do."', "  ", 300, f4);
        }, f2 = function() {
            $scope.addLine("[visitor@HVeeman " + $scope.path + "] $> ", "echo $WELCOME_MESSAGE", 100, f3);
        }, f1 = function() {
            $scope.addLine("[visitor@HVeeman " + $scope.path + "] $> ", "cd ~/app/public", 100, f2), 
            $scope.path = "app/public";
        };
        f1();
    }, $scope.init = function() {
